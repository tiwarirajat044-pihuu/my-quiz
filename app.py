...
st.markdown("""
    ...
    .btn-orange>button { background: linear-gradient(135deg, #f12711 0%, #f5af19 100%); }
...
def init_dbs():
    if not os.path.exists(DB_USERS):
        pd.DataFrame(columns=["Username", "Password", "CreatedAt", "LastLogin"]).to_csv(DB_USERS, index=False)
    if not os.path.exists(DB_LOGINS):
        pd.DataFrame(columns=["Username", "Timestamp"]).to_csv(DB_LOGINS, index=False)
...
# ==========================================
# 🧠 ADVANCED PDF PARSER (HIDES ANSWERS)
# ==========================================
def extract_questions_from_pdf(uploaded_file):
    ...
    for line in lines:
        ...
        if re.match(r"^(\d+[\.\)]|Q\d+[\.\)])", line_s):
            ...
            cur_q = re.sub(r"^(\d+[\.\)]|Q\d+[\.\)])\s*", "", line_s)
            cur_opts, cur_ans = [], None
        # Detect Options
        elif re.match(r"^([A-D][\.\)]|\([A-D]\))", line_s):
            cur_opts.append(re.sub(r"^([A-D][\.\)]|\([A-D]\))\s*", "", line_s))
        # Detect Answer (Auto-Scan & Hide)
...
# ==========================================
    ...
    tab_u, tab_qb, tab_sch, tab_res, tab_set = st.tabs([
        "👥 User Logs & Records", "📚 Question Bank (PDF/Manual)", "⚙️ Test Schedule & Control", "📊 Results Declaration", "🔑 Settings"
    ])

    # --- TAB 1: User Logs (Includes Passwords as requested) ---
    with tab_u:
    ...
    with tab_qb:
        ...
        if up_mode == "Upload PDF Question Bank":
            ...
            if pdf_file and test_name:
                ...
                if extracted:
                    st.success(f"✅ Extracted {len(extracted)} questions perfectly!")
    ...
    # --- TAB 4: Results Declaration & Analytics ---
    with tab_res:
        st.subheader("📊 Declare Results & View Participant Live Status")
        if not tests_db: st.info("No tests available.")
            ...
            if c2.button(f"❌ Cancel & Wipe Results for '{sel_test_res}'", type="primary"):
                t_data['declared'] = False
                if sel_test_res in results_db: del results_db[sel_test_res]
                save_json(DB_TESTS, tests_db)
                save_json(DB_RESULTS, results_db)
                st.error("Results wiped completely!")

            st.markdown("### 👨‍🎓 Live Participant Actions (Submitted / Incomplete / Skipped)")
            if sel_test_res in results_db and results_db[sel_test_res]:
                res_list = []
                for user, udata in results_db[sel_test_res].items():
                    res_list.append({
                        "Student": user,
                        "Status": udata['status'],
                        "Score": f"{udata['score']} / {len(t_data['questions'])}",
...
# ==========================================
    ...
    # If user is currently taking a test, show ONLY the test UI.
    if st.session_state.active_test_view:
        ...
        # Initialize user progress for this test in session
        if "progress" not in st.session_state:
            # Check if exists in DB (Resume logic)
    ...
    # ---------------------------------------------------------
    # MAIN DASHBOARD (When not actively taking a test)
    # ---------------------------------------------------------
    else:
                    ...
                    if t_name in results_db:
                        ...
                        for u, d in results_db[t_name].items():
                            if d['status'] == "Completed":
                                board.append({"Rank": 0, "Student": u, "Score": d['score'], "Time(s)": round(d['time_taken_sec'],1)})
